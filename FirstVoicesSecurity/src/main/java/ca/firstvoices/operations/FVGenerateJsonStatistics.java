...
public class FVGenerateJsonStatistics {
    ...
    private ObjectNode generateDocumentStatsJson(String docType) {
        ...
        if (query != null) {
            ...
            DocumentModelList enabledDocs = session.query(query + " AND ecm:currentLifeCycleState='Enabled'", null, 1,
                    0, true);
            documentJsonObj.put("enabled", enabledDocs.totalSize());

            // Docs in 'Published' state
            DocumentModelList publishedDocs = session.query(query + " AND ecm:currentLifeCycleState='Published'", null,
                    1, 0, true);
            documentJsonObj.put("published", publishedDocs.totalSize());

            // Docs in 'Disabled' state
            DocumentModelList disabledDocs = session.query(query + " AND ecm:currentLifeCycleState='Disabled'", null, 1,
                    0, true);
            ...
            String todayDateString = currentYear + "-" + (currentMonth < 10 ? ("0" + currentMonth) : (currentMonth))
                    + "-" + currentDay;
            ...
            String sevenDaysAgoDateString = sevenDaysAgoYear + "-"
                    + (sevenDaysAgoMonth < 10 ? ("0" + sevenDaysAgoMonth) : (sevenDaysAgoMonth)) + "-"
                    + sevenDaysAgoDay;
            ...
            DocumentModelList childrensArchiveDocs = session.query(query + " AND fv:available_in_childrens_archive=1",
                    null, 1, 0, true);
            documentJsonObj.put("available_in_childrens_archive", childrensArchiveDocs.totalSize());
            //
            // // List of most recently created docs
            // DocumentModelList recentlyCreatedDocs = session.query(query + " ORDER BY dc:created DESC", null,
            ...
            // DocumentModelList recentlyModifiedDocs = session.query(query + " ORDER BY dc:modified DESC", null,
            // maxQueryResults, 0, true);
            ...
            // DocumentModelList userMostRecentlyModifiedDocs = session.query(query + " AND dc:lastContributor='" +
            // principalName + "' ORDER BY dc:modified DESC", null, maxQueryResults, 0, true);
            ...
            // userRecentlyModifiedJsonObj.put("dc:lastContributor",
    ...
    private String constructQuery(String docType) {
        ...
        if (docType.equalsIgnoreCase("words")) {
            ...
            query = String.format(BASE_DOCS_QUERY, "FVBook", dialectPath) + proxyQueryParams
