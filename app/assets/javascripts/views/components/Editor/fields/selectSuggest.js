...
function renderInput(locals) {
    ...
    let content = <div>
        <Preview id={locals.value} expandedValue={selectn('attrs.expandedValue', locals)}
                 type={locals.type} {...previewProps} />
        {(locals.attrs.allowEdit) ? <DialogCreateForm context={locals.context} value={locals.value}
                                                      expandedValue={selectn('attrs.expandedValue', locals)}
                                                      onChange={onChange} fieldAttributes={locals.attrs}/> : ''}
    </div>;

    if (!locals.value) {
        content = <div>
            <AutoSuggestComponent locals={locals} type={locals.type} value={locals.value || ''}
                                  provider={locals.attrs.page_provider} dialect={locals.context} onChange={onChange}/>
            {(locals.attrs.hideCreate) ? '' :
                <DialogCreateForm context={locals.context} onChange={onChange} fieldAttributes={locals.attrs}/>}
            <BrowseComponent type={locals.type}
