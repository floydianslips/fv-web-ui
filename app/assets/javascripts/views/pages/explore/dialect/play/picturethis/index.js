...
export default class Picturethis extends Component {
            ...
            <div className="flex-container">
                <div className="grid">
                    {this.config.themes.map((theme, index) => {
                        return (<div key={index} className="cell" style={{cursor: 'pointer'}}
        ...
        if (wordInList !== false && wordInPicture !== false) {
            if (selectn('locationInt', wordInPicture) === selectn('locationInt', wordInList) && wordInPicture != undefined) {
                ...
                this.setState({
                    selectedWordInList: false,
                    selectedWordInPicture: false,
                    foundWordKeys: this.state.foundWordKeys.push(this.state.selectedTheme.name + '-' + selectn('locationInt', wordInList))
                })
            }
            else {
    ...
    renderGame(words) {
        ...
        if (remoteWords.size != this.state.selectedTheme.words.size) {
            warning = intl.trans('views.pages.explore.dialect.play.picture_this.warning','Note: This archive does not contain all of the words required for this game. Placeholders will be used.');
        }
        ...
        return (<div className="game">
            ...
            <div style={{position: 'relative', display: 'inline-block'}}>
                <img className="responsive-image" style={{borderRadius: '30px', border: '1px solid #000'}}
                     src={`${theme.image}`}/>
                {remoteWords.map((word, key) => {
                    ...
                    if (this.state.foundWordKeys.includes(theme.name + '-' + selectn('locationInt', word))) {
                        dot = <div style={{
                            ...unknownLocation, ...locationNumberStyle, ...foundLocation,
                            top: selectn('location.y', word) + 'px',
                            left: selectn('location.x', word) + 'px'
                        }} onMouseUp={this.selectWordOrMatch.bind(this, word, true)}>{word.locationInt + 1}</div>;
                    }
                    else {
                        dot = <div style={{
                            ...unknownLocation, ...highlight,
                            top: selectn('location.y', word) + 'px',
                            left: selectn('location.x', word) + 'px'
                        }} onMouseUp={this.selectWordOrMatch.bind(this, word, true)}></div>;
                    }
                ...
                {remoteWords.map((word, key) => {
                    ...
                    return <div key={word.locationInt}>
                        <div style={tableCellStyle}>
                            <div style={dotStyle} onMouseUp={dotAction}>{word.locationInt + 1}</div>
                        </div>
