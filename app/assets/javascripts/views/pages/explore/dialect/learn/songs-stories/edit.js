...
export default class PageDialectBookEdit extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (this._getBookPath() != null) {
      currentBook = ProviderHelpers.getEntry(
        this.props.computeBook,
        this._getBookPath()
      );
      nextBook = ProviderHelpers.getEntry(
        nextProps.computeBook,
        this._getBookPath()
      );
    }
  ...
  _getBookPath(props = null) {
    ...
    if (StringHelpers.isUUID(props.routeParams.bookName)) {
      ...
      return (
        ...
        "/Stories & Songs/" +
        StringHelpers.clean(props.routeParams.bookName)
      );
    }
  }
  ...
  componentDidUpdate(prevProps, prevState) {
    let book = selectn(
      "response",
      ProviderHelpers.getEntry(this.props.computeBook, this._getBookPath())
    );
    ...
    if (
      title &&
      selectn("pageTitleParams.bookName", this.props.properties) != title
    ) {
      this.props.changeTitleParams({ bookName: title });
      this.props.overrideBreadcrumbs({
        find: uid,
        replace: "pageTitleParams.bookName"
      });
    }
  }
  ...
  render() {
    ...
    const computeBook = ProviderHelpers.getEntry(
      this.props.computeBook,
      this._getBookPath()
    );
    const computeBookEntries = ProviderHelpers.getEntry(
      this.props.computeBookEntries,
      this._getBookPath()
    );
    const computeDialect2 = ProviderHelpers.getEntry(
    ...
    if (
      ...
      selectn("response", computeBook)
    ) {
      let providedFilter =
        selectn(
          "response.properties.fv-phrase:definitions[0].translation",
          computeBook
        ) ||
        selectn(
          "response.properties.fv:literal_translation[0].translation",
          computeBook
        );
