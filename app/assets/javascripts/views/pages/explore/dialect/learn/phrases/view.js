...
export default class View extends Component {
    ...
    render() {
        ...
        (selectn('response.contextParameters.phrase.related_audio', computePhrase) || []).map(function (audio, key) {
            audios.push(<Preview styles={{maxWidth: '350px'}} key={selectn('uid', audio)} expandedValue={audio}
                                 type="FVAudio"/>);
        })
        ...
        return <DetailsViewWithActions
            ...
            <div className="row" style={{marginTop: '15px'}}>
                    ...
                    <div>
                        ...
                        {(() => {
                            if (phrase_books.length > 0) {
                                return <span><strong>{intl.trans('phrase_books', 'Phrase Books', 'words')}</strong>: {phrase_books}</span>;
                            }
                        })()}

                        <SubViewTranslation group={selectn('response.properties.fv:definitions', computePhrase)}
                                            groupByElement="language" groupValue="translation">
                            <p><strong>{intl.trans('definitions', 'Definitions', 'first')}:</strong></p>
                        </SubViewTranslation>

                        <SubViewTranslation group={selectn('response.properties.fv:literal_translation', computePhrase)}
                                            groupByElement="language" groupValue="translation">
                            <p><strong>{intl.trans('literal_translations', 'Literal Translations', 'words')}:</strong>
                            </p>
                        </SubViewTranslation>
                ...
                <div className={classNames('col-xs-12', 'col-md-3')}>
                    ...
                    <div>{(audios.length === 0) ?
                        <span>{intl.trans('views.pages.explore.dialect.learn.words.no_audio_yet', 'No audio is available yet', 'first')}.</span> : audios}</div>
